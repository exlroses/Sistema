...
switch ($_GET['op'])
{
    case 'guardaryeditar':
        if (empty($idingreso))
        {
            $rspta = $ingreso->ingresar($idproveedor,$idusuario,$tipo_comprobante,$serie_comprobante,$num_comprobante,$fecha_hora,$impuesto,$total_compra,
    ...
    case 'anular':
        ...
        echo $rspta?"se ha anulado el ingreso":"error al anular el ingreso";
        break;
    case 'mostrar':
    ...
    case 'listarDetalle':
        ...
        $rspta = $ingreso->listarDetalles($id);
    ...
    case 'selectProveedor':
        ...
        {
            echo '
            <option value='.$reg->idpersona.'>'.$reg->nombre.'</option>
            ';
        }
        break;
    ...
    case 'listarArticulos':
        ...
        {
            $data[]= array(
                "0"=> '<button class="btn btn-warning" onclick="agregarDetalle('.$reg->idarticulo.',\''.$reg->nombre.'\')" ><span class="fa fa-plus"></span>
                       </button>',
                "1"=> $reg->nombre,
                "2"=> $reg->categoria,
                "3"=> $reg->codigo,
